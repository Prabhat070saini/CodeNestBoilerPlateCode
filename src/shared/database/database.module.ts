...
class DatabaseShutdownService implements OnApplicationShutdown {
  ...
  async onApplicationShutdown(signal?: string) {
    this.logger.log(`🧹 [DatabaseModule] Shutting down due to signal: ${signal ?? 'manual stop'}`);
    try {
      if (this.dataSource?.isInitialized) {
        await this.dataSource.destroy();
        this.logger.log('✅ Database connection closed gracefully');
      } else {
        this.logger.log('ℹ️ Database connection already closed or not initialized');
      }
    } catch (error) {
...
@Global()
@Module({
  providers: [DatabaseConfig, ...CAN_DATABASE_PROVIDER, DatabaseShutdownService],
